...
class LatentBaseDataset(Dataset):
    ...
    def _extract_latent(self, obj):
        """Extract latent tensor from various input formats"""
        if isinstance(obj, dict):
            latent = obj.get("latent", None)
            if latent is None:
                raise ValueError("Missing key 'latent' in latent dict")
            obj = latent
        ...
        raise TypeError(f"Unsupported latent type: {type(obj)}")
...
class LatentTrainingDataset(LatentBaseDataset):
    def __init__(
        ...
        super().__init__(data_folder, data_split_folder, dataset_name, image_size, min_dist_cat, max_dist_cat,
            len_traj_pred, traj_stride, context_size, traj_names, normalize, predefined_index, goals_per_obs)
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            # Load pre-encoded latents instead of raw images
            obs_latents = torch.stack([
                self._extract_latent(torch.load(os.path.join(self.data_folder, f, f"{t}.pt"), map_location='cpu')) 
                for f, t in context
            ])
...
class LatentEvalDataset(LatentBaseDataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        try:
            ...
            # Load pre-encoded latents for both context and prediction
            obs_latents = torch.stack([
                self._extract_latent(torch.load(os.path.join(self.data_folder, f, f"{t}.pt"), map_location='cpu'))
                for f, t in context
            ])
