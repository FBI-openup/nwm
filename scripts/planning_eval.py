...
"""
NOMAD Path Planning Evaluation Script
====================================

This script evaluates the quality of path planning and navigation generated by NOMAD models.
It is SEPARATE from inferEval.py which evaluates image generation quality.

FUNCTIONALITY:
- Evaluates trajectory generation accuracy using evo library
- Measures path planning performance with APE/RPE metrics  
- Analyzes navigation behavior and pose estimation quality
- Supports rollout evaluation and CEM optimization

OPTIONAL DEPENDENCIES:
This script requires specialized trajectory evaluation libraries that are NOT installed by default.

To install the required dependencies:
1. Run: pip install -r requirements-eval.txt
   OR
2. Run: python scripts/worldmem_setup_and_test.py (includes eval dependencies)

Required packages from requirements-eval.txt:
- evo>=1.12.0          (trajectory evaluation with ROS support)
- dreamsim             (advanced similarity metrics)

NOTE: These are heavyweight packages (~100MB+) and only needed for NOMAD path planning evaluation.
If you only need image generation evaluation, use inferEval.py instead.

USAGE EXAMPLES:
python scripts/planning_eval.py --exp config/nwm_config.yaml --datasets scand --save_preds
python scripts/planning_eval.py --exp config/nwm_config.yaml --datasets recon,tartan --plot
"""
...
from evo.core import sync, metrics  # type: ignore
import evo.main_ape as main_ape  # type: ignore
import evo.main_rpe as main_rpe  # type: ignore
from evo.core.metrics import PoseRelation  # type: ignore
...
from datasets import TrajectoryEvalDataset  # type: ignore
from scripts.isolated_nwm_infer import model_forward_wrapper  # type: ignore
from misc import calculate_delta_yaw, get_action_torch, save_planning_pred, log_viz_single, transform, unnormalize_data  # type: ignore
from scripts.isolated_nwm_eval import save_metric_to_disk  # type: ignore
import distributed as dist  # type: ignore
from models import CDiT_models  # type: ignore
...
def get_dataset_eval(config, dataset_name, predefined_index=True):
    ...
    if predefined_index:
        predefined_index_file = f"data_splits/{dataset_name}/test/navigation_eval.pkl"
    else:
        predefined_index_file = None
    ...
    dataset = TrajectoryEvalDataset(
                ...
                transform=transform,
                predefined_index=predefined_index_file,  # type: ignore[arg-type]  # datasets.py type annotation is incorrect
...
class WM_Planning_Evaluator:
    ...
    def generate_actions(self, dataset_save_output_dir, dataset_name, idxs, obs_image, goal_image, gt_actions, len_traj_pred):
        ...
        # Initialize variables that are used outside the loop
        losses = []
        preds = None
    ...
    @torch.no_grad
    def evaluate(self):
        # Initialize metric_logger outside the loop to avoid unbound variable error
